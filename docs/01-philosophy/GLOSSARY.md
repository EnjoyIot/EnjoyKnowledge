...
### capture
2 个核心工作流之一（W2，v0.2 砍 5→2 工作流后保留）。把对话中发现的隐性知识沉淀到 SoT 的工作流。触发：`/remember "..."` 或 AI 建议。流程：分类（gotcha/decision/pattern/rule）→ 校验必填字段 → 写 SoT → 更新索引。详见 workflows.md §4.2。

### Core
enjoyknowledge Core。Layer 2：知识管理原语层（`add` / `ls` / `grep` / `cat` / `doctor` / `export`），不写死目录名、不关心领域语义。
...
### onboard
2 个核心工作流之一（W1，v0.2 砍 5→2 工作流后保留）。AI 工具首次进入仓库时建立项目心智模型。触发：AI 工具启动 / `enjoyknowledge onboard`。流程：读 AGENTS.md 路由表 → 读 POSITIONING → 读 DESIGN-PHILOSOPHY → 读 DESIGN → 读 gotchas（severity 4-5）→ 读 active decisions。详见 workflows.md §4.1。
