...
### doctor
`enjoyknowledge doctor`。健康检查命令。v4 检查：缺 frontmatter / 缺必填字段（如 gotcha 的 `trigger`）/ SoT 过期 / Rule-Code 不一致 / **多工具 export** 一致性。
...
### for Coding
enjoyknowledge 基于 Core 的第一个领域应用，面向 AI 编程场景。定义默认知识结构（10 类资产）+ **v0.2 多工具入口（首发 Claude + Cursor）** + 5 核心工作流。**v4 不再硬编码目录**（v3 写死 `architecture/` / `gotchas/` 等被废弃）——目录名是路由表的一个约定项，可由用户配置。
...
### sync
5 个核心工作流之一（W5）。把 SoT 导出到 v0.2 首发 2 个 AI 工具（Claude + Cursor）的入口文件。触发：`enjoyknowledge export` / `on_post_record`。流程：读 tools.yaml → 渲染每个工具 → 校验幂等。详见 workflows.md §4.4。
...
- **2026-06-27 v4.2 修订**：v0.2 工具 9→2（**首发 Claude + Cursor**，非 9→1 因 1 工具 = 杀 thesis；架构保留 9 工具 adapter）。命令 sync → export（1 工具时 sync 撒谎，export 诚实）。9 工具入口词条改"v0.2 多工具入口"。doctor 检查项"9 工具同步一致性"改"多工具 export 一致性"。AGENTS.md 词条"sync 自动生成"改"export 自动生成"。
