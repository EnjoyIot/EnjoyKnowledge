...
### `.enjoyknowledge/`
enjoyknowledge 长期知识库的根目录。位于项目根目录下。包含类型子目录（architecture/、gotchas/、patterns/、business/、decisions/）和 OKF 保留文件（index.md）。只放已审核、可复用、值得推送给 AI 的知识；短期任务材料放在并列的 `knowledge-tasks/`。
...
### doctor
`enjoyknowledge doctor`。健康检查命令。5 项检查：缺 frontmatter、缺 description、超出预算（>20 条）、AGENTS.md 过期、待归档任务。
...
### frontmatter
Markdown 文件开头的 YAML 元数据块，以 `---` 分隔。使用 OKF 兼容格式：`title`、`description`、`tags`、`timestamp`。目录名即分类，无需 `type` 字段。
...
### index.md
OKF 保留文件名。目录的目录（table of contents），列出本目录下所有概念文件及其 description。可选存在，由系统生成和维护。
...
### rules/
`.enjoyknowledge/rules/` 目录。项目规则的唯一真值源（工具无关 Markdown）。三层防护设计：① 源规则层 — 每条规则强制带「适用范围」标注（自然语言声明适用文件类型/语言/框架），`doctor` 检查缺标注 → 报 warning；② 推送层 — `add` 时自动更新 AGENTS.md `RULES` 块，按语言/框架分区（通用、前端、Rust 后端、API 等），规则全量推送但分区提供强信号，AI 自行判断适用性；③ 兜底层 — `doctor` 检查源规则与 AGENTS.md 推送块一致性。工具特有规则（Cursor `globs`、Claude system prompt 等）留在各自原生文件，不进入统一管理——不试图统一天然不该统一的东西。
