...
### capture
2 个核心工作流之一（W2，v0.2 砍 5→2 工作流后保留）。把对话中发现的隐性知识沉淀到 SoT 的工作流。**CLI 接入**：`enjoyknowledge workflow capture`（v0.2.1 实现）。触发：`/remember "..."` 或 AI 建议。流程：分类（gotcha/decision/pattern/rule）→ 校验必填字段 → 写 SoT → 更新索引。详见 workflows.md §4.2。**已知 v0.2.1 限制**：`capture` 自动路由用大写文件名（`GOTCHAS.md` / `DECISIONS.md` 等），与 `init` 生成的小写（`gotchas.md` / `decisions.md`）不一致——A3 修。
