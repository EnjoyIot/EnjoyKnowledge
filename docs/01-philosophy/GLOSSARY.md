...
### capture
5 个核心工作流之一（W2）。把对话中发现的隐性知识沉淀到 SoT 的工作流。触发：`/remember "..."` 或 AI 建议。流程：分类（gotcha/decision/pattern/rule）→ 校验必填字段 → 写 SoT → 更新索引。详见 workflows.md §4.2。
...
### sync
5 个核心工作流之一（W3）。把 SoT 导出到 v0.2 首发 2 个 AI 工具（Claude + Cursor）的入口文件。触发：`enjoyknowledge export` / `on_post_record`。流程：读 tools.yaml → 渲染每个工具 → 校验幂等。详见 workflows.md §4.3。
