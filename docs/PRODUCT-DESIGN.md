...
AGENTS.md                    ←  主文件，始终生成。所有工具的通用入口
    │
    └── "详见 .enjoyflow/snapshots/<REQ-ID>.md"
...
- 知识库: `.enjoyflow/knowledge-base/`
- 快照: `.enjoyflow/snapshots/`

当用户提到 "修 bug"、"新功能"、"重构" 等任务时：
1. 检查 `.enjoyflow/snapshots/` 下是否有对应快照
2. 如果没有，提示用户运行：
   ```bash
   enjoyflow snapshot <REQ-ID> --preset <scenario>
   ```
3. 快照包含架构、契约、踩坑、进度——先读完再动手

可用命令：
- `enjoyflow snapshot <REQ-ID>` — 生成任务上下文快照
...
| **L1 任务** | Skill 教学 → AI 读快照或调 CLI | ContextFlow 快照（任务级上下文） | 当前任务 |
| **L2 兜底** | 人手动 `enjoyflow snapshot` 然后 @file | 任意场景的完整上下文 | 任意 AI 工具 |
...
AI: [读到 AGENTS.md 中的 EnjoyFlow 指令]
    → 场景识别: bug_fix
    → 检查 .enjoyflow/snapshots/ 下是否有快照
    → 没有快照，提示:

    "运行 enjoyflow snapshot REQ-042 --preset bug_fix"
```

### 4.2 快照生成后

```
$ enjoyflow snapshot REQ-042 --preset bug_fix
✓ 架构上下文: backend/export 模块
✓ 业务规则: 单次最多 10 万行
✓ 已知踩坑: t_export_record 没有 status 字段
✓ 当前进度: IN_PROGRESS
✓ 快照已生成: .enjoyflow/snapshots/REQ-042.md
```
...
### 2. 获取上下文
运行 `enjoyflow snapshot <REQ-ID> --preset <scenario>` 或读取已有快照
...
### 4. 任务结束后
提醒用户运行 `enjoyflow snapshot <REQ-ID>` （刷新进度）
提醒用户更新知识（如有新发现的坑/模式）
```
...
paths:
  ...
  lifecycle: ".enjoyflow/lifecycle"
  snapshots: ".enjoyflow/snapshots"
```
...
│   │
│   ├── lifecycle/
│   │   └── active-sprint/       # D1 任务进度
│   │
│   ├── snapshots/               # ContextFlow 快照（本地，不进 git）
│   │   └── REQ-042.md
│   │
│   └── skills/
...
`.enjoyflow/snapshots/` 是本地产物，每个人跑自己的快照。进 git 的是 `.enjoyflow/knowledge-base/`——那是源。快照是源的视图，不是源本身。
...
enjoyflow snapshot <REQ-ID> [--preset] # 生成上下文快照
enjoyflow show <REQ-ID>                # 查看任务状态
