...
`index.md` 是 OKF 保留文件，由 enjoyknowledge 生成和维护。不属于任何知识分类。
...
`enjoyknowledge doctor` 是知识文件的 linter，5 项检查：
...
模板是一个可重用的知识目录骨架，`init --template <name>` 可以直接展开到项目中。enjoyknowledge 内置 `for-coding` 默认模板，同时支持用户自定义模板。

#### 9.1.1 默认模板：for Coding

`enjoyknowledge init`（不带 `--template`）使用内置 `for-coding` 模板，生成 5 个长期知识目录：

```
.enjoyknowledge/
├── architecture/              # 架构设计、模块划分、数据流
│   └── overview.md            # 种子文件：项目架构模板
├── gotchas/                   # 踩坑记录、边界案例、变通方案
│   └── gotchas.md             # 种子文件：踩坑记录模板
├── patterns/                  # 经过验证的模式和最佳实践
│   └── patterns.md            # 种子文件：模式记录模板
├── business/                  # 业务规则、计算公式、合规要求
│   └── business.md            # 种子文件：业务规则模板
├── decisions/                 # 架构决策记录 (ADR)
│   └── 000-template.md        # 种子文件：ADR 模板
└── index.md                   # 目录的目录（OKF 保留）
```

外加项目根目录的：
- `AGENTS.md` — AI 入口文件（含知识摘要推送块）
- `knowledge-tasks/` — 任务过程材料暂存区（与长期知识并列）

每个种子文件包含标准 frontmatter、结构说明和填充模板，用户可直接编辑而无需学习格式。

**种子文件内容说明**：

| 种子文件 | 提供什么 | 用户填什么 |
|---|---|---|
| `architecture/overview.md` | 模块地图表格、技术栈表格、数据流图模板、部署拓扑模板 | 项目实际的模块划分、技术选型与理由、数据流向、部署方式 |
| `gotchas/gotchas.md` | 踩坑条目模板（现象-影响-方案-关联） | 开发中实际踩过的坑、边界案例、变通方案 |
| `patterns/patterns.md` | 模式条目模板（场景-做法-理由-反模式） | 经过验证的编码模式、最佳实践 |
| `business/business.md` | 业务规则模板（规则-范围-代码位置-测试） | 领域规则、计算公式、合规要求 |
| `decisions/000-template.md` | ADR 模板（状态-上下文-决策-后果） | 影响架构的关键技术决策 |

所有种子文件使用 `__TIMESTAMP__` 占位符，`init` 时自动替换为当前日期。

#### 9.1.2 自定义模板

创建自定义模板只需在模板目录下放一个骨架：

```
~/.enjoyknowledge/templates/<name>/
├── .enjoyknowledge/           # 目录骨架（直接复制到项目）
│   ├── contracts/
│   │   └── template.md
│   └── regulations/
│       └── template.md
├── knowledge-tasks/           # 可选，任务暂存区骨架
└── _AGENTS.md                 # 可选，AGENTS.md 模板
```

引擎看到 `contracts/` 目录推导分类为 Contract，看到 `regulations/` 推导分类为 Regulation——不需要注册、不需要配置、不需要改引擎代码。这条来自 §8.1"文件系统即索引"：目录名承载语义，引擎只做推导，不写死分类。

`_AGENTS.md` 文件可选；不存在时使用内置 AGENTS.md 模板。可使用 `{{TEMPLATE_NAME}}` 占位符，应用时自动替换为模板名。

模板加载优先级：
1. `.enjoyknowledge/templates/<name>/`（项目级，随项目 git 管理）
2. `~/.enjoyknowledge/templates/<name>/`（用户级，跨项目共享）
...
cp -r .enjoyknowledge/{architecture,gotchas,patterns,business,decisions} \
     ~/.enjoyknowledge/templates/my-template/.enjoyknowledge/
cp AGENTS.md ~/.enjoyknowledge/templates/my-template/_AGENTS.md
# 编辑 _AGENTS.md，用 {{TEMPLATE_NAME}} 替换模板名占位符

# 查看所有可用模板
enjoyknowledge init --template list

# 应用模板
enjoyknowledge init --template my-template
```

**模板的本质是文件目录**——放入 git 即可版本管理：

```bash
# 团队共享模板（随项目 git 管理）
mkdir -p .enjoyknowledge/templates/team-backend
cp -r ~/.enjoyknowledge/templates/my-template/* .enjoyknowledge/templates/team-backend/
git add .enjoyknowledge/templates/ && git commit -m "添加团队后端模板"
```

自定义模板的目录名即模板名。模板加载优先级确保项目级模板覆盖用户级模板，方便团队统一规范。
