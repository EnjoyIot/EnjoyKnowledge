...
```bash
enjoyflow record gotcha --task REQ-042 --tag excel --content "t_export_record 表无 status 字段"
...
输入: gotcha, tag=excel, content="...", task=REQ-042
        ...
        ▼
[1] 路由到目标文件
    gotcha (--task) → .enjoyflow/knowledge-tasks/<REQ>/gotchas.md
    gotcha (无task) → .enjoyflow/knowledge-base/development/GOTCHAS.md
    pattern         → .enjoyflow/knowledge-base/development/PATTERNS.md
    decision        → .enjoyflow/knowledge-tasks/<REQ>/adr.md
        │
        ▼
[2] 格式化条目
    - excel: t_export_record 表无 status 字段（REQ-042, 2026-06-21）
    → 追加到 .enjoyflow/knowledge-tasks/REQ-042-export-excel/gotchas.md
        │
        ▼
[3] 追加到文件末尾
    - 不改动已有内容
    - 更新 frontmatter 的 last_modified 日期
        ...
        ▼
[4] 增量更新索引
    .index.json: 追加 "excel" → knowledge-tasks/REQ-042-export-excel/gotchas.md 映射（如新增）
```
...
| 文件 | 上限 | 超标时 |
|---|---|---|
| knowledge-base/development/GOTCHAS.md | 100 行 | 最老的一半 → `knowledge-tasks/archive/gotchas-YYYYQQ.md` |
| knowledge-base/development/PATTERNS.md | 100 行 | 同上 |
| knowledge-base/project/ARCHITECTURE.md | 150 行 | 旧内容移到 knowledge-tasks/archive/ |

归档时自动生成摘要索引：

```
knowledge-tasks/archive/INDEX.md
...
AI 运行: enjoyflow record gotcha --tag excel --content "..."
                │
                ▼
         路由 → knowledge-base/development/GOTCHAS.md（或 knowledge-tasks/<REQ>/gotchas.md）
                │
                ▼
         追加条目（不改已有）
                │
