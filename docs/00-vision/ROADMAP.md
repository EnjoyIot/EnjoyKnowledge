...
| v0.4.2 | **当前** (2026-06-28) | ek fix 保留 frontmatter 字段 + 文档重写 |
...
| `grep` | 结构化搜索（定位到 `##` 段） |
| `add` | 追加/创建知识条目 |
| `doctor` | 4 项健康检查（`--ci` 模式 warning 也非零退出） |
| `fix` | 自动修复（`--req <REQ-ID>`） |
...
`gotcha` / `decision` / `pattern` / `rule` / `business` / `architecture` / `contract` / `convention` / `context` / `template`
