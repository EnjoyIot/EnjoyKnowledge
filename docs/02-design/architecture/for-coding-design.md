...
| # | 原则 | 含义 |
|---|---|---|
| 1 | **SoT 单一 + 入口多元** | 唯一真值源是 `.enjoyknowledge/`，9 个 AI 工具各走各的入口 |
...
```
Layer 1: 9 工具入口（skills/mdc/rules/instructions）
   ↓ sync
...
```
.enjoyknowledge/
├── rules/         # 约束性 → sync 到 9 工具（必含 applies_to）
├── templates/     # 范式性 → sync 到 9 工具
├── knowledge/
...
| 5 | **Sync to AI Tools** | 改 rule/加 knowledge | 9 工具一次同步 | 同步不完整 |
...
| # | 风险 | 缓解 |
|---|---|---|
| R7 | 9 工具入口"长得一样但实际不同"的同步 bug | 不允许 generic fallback / doctor 加工具 spec 校验 |
