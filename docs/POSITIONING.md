...
ContextFlow 的本质是 `enjoyflow search` + `enjoyflow record` 命令——AI 在会话中主动调用，实时获取与当前任务相关的所有知识。不依赖持久缓存文件，每次检索都是最新的：

- **实时准确**——每次 search 都是最新知识，无缓存过期问题
- **可追溯**——`enjoyflow record` 留下的记录可审计
- **跨会话一致**——同一份 knowledge-base/ + knowledge-tasks/ 源，不同会话看到相同知识

**关键区分**：search 是实时查询，不是缓存。源是 `knowledge-base/` + `knowledge-tasks/` 里的知识文件；`enjoyflow search` 按条件实时检索聚合。详见 [SYSTEM-DESIGN.md](SYSTEM-DESIGN.md) §3.2。

```yaml
# enjoyflow search "REQ-001" 的输出示例
